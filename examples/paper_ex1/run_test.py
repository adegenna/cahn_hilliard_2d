...
def generate_left_to_right_horizontal_laser_path( amp , sigma_temp , nx , ny , num_changes ):
    
    T_amp       = amp * np.ones( num_changes )
    T_x         = np.linspace( 0 , nx + sigma_temp , num_changes )
...
def rotate_grid( xx , yy , th ):
    ...
    xxR = np.reshape( xR , [nx,ny] )
    yyR = np.reshape( yR , [nx,ny] )
    
    return xxR , yyR

def generate_random_sinusoid_IC( nx , ny , A_range , T_range ):
    
    xx,yy = np.meshgrid( np.arange( nx ) , np.arange( ny ) )
    A     = np.random.uniform( A_range[0] , A_range[1] )
    T     = np.random.uniform( T_range[0] , T_range[1] , 2 )
    ph    = np.random.uniform( 0 , np.pi , 2 )
    th_R  = np.random.uniform( 0 , np.pi )
    
    xxR,yyR = rotate_grid( xx , yy , th_R )
    
    y     = A * np.sin( xxR * 2*np.pi/(nx/T[0]) + ph[0] ) * np.sin( yyR * 2*np.pi/(ny/T[1]) + ph[1] )
    
    return y

def write_m_to_petscinputfile( m , petscfilename , petscfilename_out ):
    
    fin  = open( petscfilename , 'rt' )
    data = fin.read()
    data = data.replace( '-CH_m' , '-CH_m ' + str(m) )
    fin.close()
    fout = open( petscfilename_out , 'wt' )
    fout.write( data )
    fout.close()
...
def main():
    ...
    # Random avg mass concentration
    
    m = np.random.uniform( 0.0 , 0.25 )
    write_m_to_petscinputfile( m , 'petscrc_template.dat' , 'petscrc.dat' )
    ...
    sigma_temp                  = 0.5 * settings.nx
    amp_temp                    = 1.25
    T_amp , T_x , T_y , T_sigma = generate_left_to_right_horizontal_laser_path( amp_temp , sigma_temp , settings.nx , settings.ny , num_changes )
    ...
    initial_U      = generate_random_sinusoid_IC( settings.nx , settings.ny , [0.1,0.2] , [1.0,10.0] )
    ...
    while True:
        ...
        sim_done2  = os.path.exists( 'complete' + timestamp_final )
        if (sim_done or sim_done2):
